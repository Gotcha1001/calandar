...
export async function createEvent(data) {
  ...
  //2 validate the data according to the schema , optional step
  ...
  // 4. create the event if the user is found
...
export async function deleteEvent(eventId) {
  ...
  // check if its inside our data base or not
  const user = await db.user.findUnique({
    where: { clerkUserId: userId },
  });
  ...
  // then delete the event if found and matches
...
export async function getEventAvailability(eventId) {
  ...
  for (let date = startDate; date <= endDate; date = addDays(date, 1)) {
    const dayOfWeek = format(date, "EEEE").toUpperCase(); // Changed from "EEE" to "EEEE" to match "MONDAY"
...
function generateAvailableTimeSlots(
  ...
  while (currentTime < slotEndTime) {
    ...
    currentTime = addMinutes(slotEnd, timeGap); // Use timeGap between slots
  }
  return slots;
}
